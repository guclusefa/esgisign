...
class UserRepository extends ServiceEntityRepository implements PasswordUpgraderInterface
{
    ...
    private function findByVerified(QueryBuilder $queryBuilder, array $isVerifieds): QueryBuilder
    {
        if (count($isVerifieds) === 2) {
            return $queryBuilder;
        }
        return $queryBuilder
            ->andWhere('u.isVerified = :isVerified')
            ->setParameter('isVerified', $isVerifieds[0]);
    }

    private function findByBanned(QueryBuilder $queryBuilder, array $isBanneds): QueryBuilder
    {
        if (count($isBanneds) === 2) {
            return $queryBuilder;
        }
        return $queryBuilder
            ->andWhere('u.isBanned = :isBanned')
            ->setParameter('isBanned', $isBanneds[0]);
    }
